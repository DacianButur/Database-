drop database if exists farmaceuticalwarehouse;
create database if not exists FarmaceuticalWareHouse;
use farmaceuticalWareHouse;
...
alter table product add foreign key(idCategory) references category(idCategory) on delete set null;
